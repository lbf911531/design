...
export default {
  ...
  computed: {
    ...mapGetters(["unSchoolList", "curUserInfo"]),
    unSchoolValueList() {
      const that = this;
      return this.unSchoolList.filter(function(item) {
        if (item.name) {
          return (
  ...
  methods: {
    ...
    getUniversityClazzData() {
      this.loading = true;
      const that = this;
      this.findUniversityData("schoolmate")
        .then(data => {
          that.loading = false;
        })
        .catch(err => {
