  ...
  <div class="juniorFriendsInfo">
    ...
    >
      ...
      <el-table-column prop="relationship" label="关系" align="center">
        <template>
          <el-tag type="warning" disable-transitions>{{'好友'}}</el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  computed: {
    ...
    juniorFsList() {
      const that = this;
      return this.juniorFs.filter(function(item) {
        if (item.name) {
  ...
  created() {
    this.loading = true;
    const that = this;
    this.findJuniorListByRel("friend")
      .then(data => {
        that.loading = false;
      })
      .catch(err => {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.$confirm(`是否将 ${row.name} 移出好友列表`, "提示", {
